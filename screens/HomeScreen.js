  ...
  return (
    <View style={{ flex: 1, marginTop: 50, backgroundColor: COLORS.secondary }}>
      <View style={{ flex: 1.5, paddingHorizontal: 25, paddingVertical: 15 }}>
        <Text style={styles.headerLine1}>for the love</Text>
      ...
      >
        {Data?.menu.map((item, index) => {
          return (
            <TouchableOpacity onPress={() => tabSelect(index)} key={index}>
              <MenuItem
                icon={active === index ? item.menuIcon : item.bwIcon}
                menu={item.menuTitle}
      ...
      <View style={{ flex: 8, paddingBottom: 100 }}>
        <FlatList
          ...
          ListEmptyComponent={
            ...
            >
              <Text style={{ fontWeight: "700" }}>Sorry, nothing here</Text>
            </View>
          }
        />
      </View>
